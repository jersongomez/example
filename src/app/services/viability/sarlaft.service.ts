...
import { DefaultRs } from 'src/app/models/gosign-middleware/default-rs.model';
import { BizagiData } from 'src/app/models/gosign-middleware/bizagi-data.model';
...
import { BizagiCase } from 'src/app/models/gosign-middleware/bizagi-case.model';
import { DocumentPdf, DocumentsPdfRs } from 'src/app/models/gosign-middleware/documents-rs.model';
import { GosignSignatureRq, GosignSignatureRs } from 'src/app/models/gosign-middleware/gosign-signature.model';
import { RequestSarlaft } from 'src/app/models/index';
...
export class SarlaftService {
  ...
  constructor(
    ...
    protected viabilityService: ViabilityService,
  ...
  public fillInitialRq(): void {
    ...
    const feeRq = this.calculatorService.Request;
    ...
    this.request.creditInfo.requestAmount = fee.montoAFinanciar;
    this.request.creditInfo.purchaseValue = feeRq.valorProducto;
    this.Request.creditInfo.valueCuota = fee.valorCuotaConSeguro;
    this.request.creditInfo.insurancePolicy = feeRq.riskInsurance;
    this.request.creditInfo.gracePeriod = feeRq.gracePeriod;
    this.request.creditInfo.allRisk = fee.valorTodoRiesgo;
    this.request.creditInfo.amountInstallments = fee.numeroCuota;
    ...
    this.request.creditInfo.valueAssistance = parseInt(`${fee.valorAsistencia}`);
    this.request.creditInfo.valueSecureNoAssistance = parseInt(`${fee.valoSeguroSinAsistencia}`);
    //this.request.creditInfo.tasaEA = this.bizagiEnumTasas.obtenerTasaBizagi(this.transformDecimal(fee.nominalMesVencido * 100))

    if (feeRq.gracePeriod) {
      this.partnerFacade.gracePeriod$
        .pipe(take(1))
        .subscribe((lack) => (this.request.creditInfo.lack = lack))
        .unsubscribe();
    }

    this.request.creditInfo.totalSafe = fee.valorTotalSeguro;
  ...
  private fillParamsBizagi() {
    ...
    if (this.calculatorService.Request.subsidized) {
      this.partnerFacade
        .bizagiRateByFeeParam(this.request.creditInfo.amountInstallments)
        .pipe(take(1))
